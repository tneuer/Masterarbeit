...
class CWGAN(ConditionalGenerativeModel):
    ...
    def train(self, x_train, y_train, x_test=None, y_test=None, epochs=100, batch_size=64,
              gen_steps=1, critic_steps=5, steps=None, log_step=3, gpu_options=None):
...
if __name__ == '__main__':
    ...
    z_dim = 128
    label_dim = 10
    cwgan = CWGAN(x_dim=inpt_dim, y_dim=label_dim, z_dim=z_dim, last_layer_activation=tf.nn.sigmoid,
                  gen_architecture=gen_architecture, critic_architecture=critic_architecture,
                  folder="../../Results/Test/CWGAN_conv", image_shape=image_shape, append_y_at_every_layer=False)
    print(cwgan.show_architecture())
    cwgan.log_architecture()
    cwgan.compile(logged_images=x_train_log, logged_labels=y_train_log)
    cwgan.train(x_train, y_train, x_test, y_test, epochs=100, critic_steps=5, gen_steps=1, log_step=3)