...
from building_blocks.layers import reshape_layer, logged_dense, resize_layer, image_condition_concat
from building_blocks.networks import ConditionalGenerator, Encoder, Decoder
from building_blocks.generativeModels import ConditionalGenerativeModel
from functionsOnImages import padding_zeros


class CBEGAN(ConditionalGenerativeModel):
    ...
    def compile(self, logged_images=None, logged_labels=None, learning_rate_gen=0.0001, learning_rate_ae=0.0001, optimizer=tf.train.AdamOptimizer):
        self._define_loss()
    ...
    def train(self, x_train, y_train, x_test=None, y_test=None, epochs=100, batch_size=64, log_step=3, steps=None, gpu_options=None):
...
if __name__ == '__main__':
    ...
    enc_dim = z_dim
    label_dim = 10
    cbegan = CBEGAN(x_dim=inpt_dim, y_dim=label_dim, z_dim=z_dim, enc_dim=enc_dim, last_layer_activation=tf.nn.sigmoid,
                  gen_architecture=gen_architecture, enc_architecture=enc_architecture, dec_architecture=dec_architecture,
                  folder="../../Results/Test/CBEGAN", image_shape=image_shape, append_y_at_every_layer=False)
    print(cbegan.show_architecture())
    cbegan.log_architecture()
    cbegan.compile(logged_images=x_train_log, logged_labels=y_train_log)
    cbegan.train(x_train, y_train, x_test, y_test, epochs=100, log_step=3, gpu_options=None)