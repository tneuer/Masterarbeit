...
class CVAEGAN(Image2ImageGenerativeModel):
    def __init__(self, x_dim, y_dim, z_dim, enc_architecture, gen_architecture, adversarial_architecture, folder="./CVAEGAN",
        ...
        self._adversarial = Discriminator(self._adv_architecture, name="Discriminator")
    ...
    def compile(self, loss, optimizer, learning_rate=None, learning_rate_enc=None, learning_rate_gen=None, learning_rate_adv=None,
        ...
        with tf.name_scope("Optimizer"):
            ...
            self._adv_optimizer = disc_optimizer.minimize(self._adv_loss, var_list=self._get_vars("Discriminator"), name="Discriminator")
            ...
            self._adv_grads_and_vars = disc_optimizer.compute_gradients(self._adv_loss, var_list=self._get_vars("Discriminator"))
    ...
    def _define_loss(self, loss, label_smoothing, lmbda, feature_matching):
        ...
        with tf.name_scope("Loss") as scope:
            ...
            tf.summary.scalar("Discriminator", self._adv_loss)
    ...
    def train(self, x_train, y_train, x_test, y_test, epochs=100, batch_size=64, disc_steps=5, gen_steps=1,
        ...
        self._monitor_dict = {
            ...
            "Output Adversarial": [
                    [self._output_adv_real, self._output_adv_fake_from_real, self._output_adv_fake_from_latent],
                    ["Real", "Fake_from_real", "Fake_from_latent"],
                    [[] for i in range(3)],
                    [np.mean]
            ]
        }
        ...
        for epoch in range(epochs):
            ...
            print("\nEpoch {}: D: {}; G: {}; E: {}.".format(epoch, disc_loss_epoch, gen_loss_epoch, enc_loss_epoch))
    ...
    def _optimize(self, dataset, batch_size, disc_steps, gen_steps):
        for i in range(disc_steps):
            ...
            _, disc_loss_batch = self._sess.run([self._adv_optimizer, self._adv_loss], feed_dict={
    ...
    def plot_samples(self, inpt_x, inpt_y, sess, image_shape, epoch, path):
        ...
        image_matrix = np.array([
            [
                ...
                np.zeros(shape=(image_shape[0], image_shape[1])),
    ...
    def evaluate(self, true, condition, epoch):
        ...
        for k, key in enumerate(self._monitor_dict):
            ...
            for kr, key_result in enumerate(key_results):
                ...
                axs[k].plot(self._batches, self._monitor_dict[key][2][kr], label=self._monitor_dict[key][1][kr])
            axs[k].legend()
            axs[k].set_title(key)
            print("; ".join([
                "{}: {}".format(name, np.round(val[-1], 11))
...
if __name__ == '__main__':
    ...
    gen_architecture = [
                        [conv2d_logged, {"filters": 32, "kernel_size": 5, "strides": 2, "activation": tf.nn.leaky_relu, "padding": "same"}],
                        [conv2d_transpose_logged, {"filters": 16, "kernel_size": 5, "strides": 2,
                                                    "activation": tf.nn.leaky_relu, "padding": "same"}],
                        [conv2d_logged, {"filters": 1, "kernel_size": 5, "activation": tf.nn.sigmoid, "padding": "same"}]
                        ]
    adversarial_architecture = [
                        [conv2d_logged, {"filters": 4, "kernel_size": 5, "strides": 6, "activation": tf.nn.sigmoid, "padding": "same"}],
                        [conv2d_logged, {"filters": 8, "kernel_size": 5, "strides": 4, "activation": tf.nn.sigmoid, "padding": "same"}],
                        ]


    learning_rate = 0.0005
    learning_rate_adv = 0.000001
    batch_size = 16
    epochs = 30
    gen_steps = 10
    disc_steps = 1
    ...
    batch_log_step = 300 #int(len(x_train) / batch_size / 10)
    ...
    save_folder = init.initialize_folder(algorithm="CVAEGAN_", base_folder=save_folder)

    cvaegan = CVAEGAN(x_dim=inpt_dim, y_dim=opt_dim, z_dim=z_dim, enc_architecture=enc_architecture,
    ...
    print(cvaegan._generator.get_number_params())
    print(cvaegan._adversarial.get_number_params())
    print(cvaegan._encoder_mean.get_number_params())
    nr_params = cvaegan.get_number_params()
    print(cvaegan.show_architecture())
    with open(save_folder+"/config.json", "w") as f:
        json.dump(config_data, f, indent=4)
    cvaegan.log_architecture()
    cvaegan.compile(loss=loss, learning_rate=learning_rate, learning_rate_adv=learning_rate_adv, optimizer=tf.train.AdamOptimizer,
                    lmbda=lmbda, feature_matching=feature_matching, label_smoothing=label_smoothing)
    cvaegan.train(x_train, y_train, x_test, y_test, epochs=epochs, batch_size=batch_size,
                   disc_steps=disc_steps, gen_steps=gen_steps, log_step=1, batch_log_step=batch_log_step, gpu_options=gpu_options)
