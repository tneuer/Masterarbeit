...
class TrainedCycleGenerator():
    ...
    def generate(self, x_inputs, y_inputs=None):
        n = len(x_inputs)
        is_training = self._graph.get_tensor_by_name("Inputs/is_training:0")
        x_input = self._graph.get_tensor_by_name("Inputs/x:0")
        y_input = self._graph.get_tensor_by_name("Inputs/y:0")
        generator = self._graph.get_tensor_by_name(self._generator_out)
        output = self._sess.run(generator,
                                feed_dict={x_input: x_inputs, is_training: False}
                ).reshape([n, self._image_shape[0], self._image_shape[1]])
        return output
    ...
    def generate_mean_from_condition(self, inputs, n):
        ...
        for i, inpt in enumerate(inputs):
            ...
            outputs[i] = np.mean(self.generate(repetitions), axis=0)
        outputs = np.stack(outputs, axis=0)
        return outputs

    def generate_overlay(self, inputs):
        output = self.generate(inputs=inputs)
        added_image = np.sum(output, axis=0)
        return added_image

    def generate_multiple(self, list_of_inputs):
        outputs = [self.generate(x_inputs=inputs) for inputs in list_of_inputs]
        return outputs

    def generate_multiple_overlay(self, list_of_inputs):
    ...
    def build_simulated_events(self, condition, tracker_image, calo_image, eval_functions, gen_scaler=1, n=10, title=None, reference_images=None):
        ...
        outputs = self.generate_multiple_overlay(list_of_inputs=inputs)*gen_scaler
        ...
        fig, ax = plt.subplots(nrows=layout[0], ncols=layout[1], figsize=(layout[1]*6, layout[0]*6))
